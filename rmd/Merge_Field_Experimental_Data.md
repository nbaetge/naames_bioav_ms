...
``` r
bge <- read_rds("~/GITHUB/naames_bioav_ms/Output/processed_bge_summary.rds") %>% 
  select(Season, Station, contains("ave_bge"), contains("sd_bge"), contains("ccf")) %>% 
  distinct()
...
bcd <- interpolated.df %>% 
  ...
  fill(c(ave_bge, ave_bge_cruise, sd_bge, sd_bge_cruise, ave_ccf_cruise, sd_ccf_cruise), .direction = "downup") %>% 
  ungroup() %>% 
  group_by(Cruise) %>% 
  mutate(ave_bge_station = ifelse(is.na(ave_bge_station), ave_bge_cruise, ave_bge_station),
         ave_ccf_station = ifelse(is.na(ave_ccf_station), ave_ccf_cruise, ave_ccf_station)) %>% 
  ungroup() %>% 
  mutate(bcd_cruise = round(interp_bp/ave_bge_cruise),
         bcd_station = round(interp_bp/ave_bge_station)) %>%  #units are  µmol C / m3 / d, bp in nmol C / L / d is equivalent to  µmol C / m3 / d 
  mutate(bc_cruise = interp_ba * (10^3) * ave_ccf_cruise * (1/10^15) * (1/12) * 10^6,
         bc_station = interp_ba * (10^3) * ave_ccf_station * (1/10^15) * (1/12) * 10^6) # bact carbon in µmol C/m3
```
...
int_bcd <- bcd %>% 
  group_by(CampCN) %>% 
  filter(Depth <= Ez) %>% 
  mutate(int.bcd_cruise = integrateTrapezoid(Depth, bcd_cruise, type = "A"),
         int.bcd_station = integrateTrapezoid(Depth, bcd_station, type = "A"),
         int.bp = integrateTrapezoid(Depth, interp_bp, type = "A"),
         int.ba = integrateTrapezoid(Depth, interp_ba, type = "A"),
         int.bc_cruise = integrateTrapezoid(Depth, bc_cruise, type = "A"),
         int.bc_station = integrateTrapezoid(Depth, bc_station, type = "A")) %>% 
...
exp_doc <- read_rds("~/GITHUB/naames_bioav_ms/Output/processed_bge.rds") %>% 
  ...
  mutate(doc_star = ifelse(Hours < stationary.harvest, interp_ptoc - boc_i.ccf, interp_ptoc - boc_s.ccf),
         ...
         sd_combined_doc = ifelse(is.na(sd_combined_doc) & Hours == 0, sd_ptoc, sd_combined_doc))  %>%  
