...
bact_npp <- read_rds("~/GITHUB/naames_bioav_ms/Input/master/processed_bf.2.2020.rds") %>%
  ...
  select(Cruise:Station, degree_bin, CampCN, Z_MLD,  Target_Z, Temperature, Temperature1, Fluorescence, BactProd_C, BactProd_C_sd, BactAbund, BactAbund_sd) %>%  
  mutate(temp = (Temperature + Temperature1) / 2) %>% 
  select(Cruise:Target_Z, temp, everything(), -contains("Temperature")) %>% 
  mutate(Cruise = gsub("AT39-6", "AT39", Cruise)) %>% 
  mutate(degree_bin = ifelse(Cruise == "AT34" & Station == 4, 48, degree_bin)) %>% 
  mutate_at(vars(BactProd_C:BactProd_C_sd), round, 2) %>% 
  filter(!is.na(BactProd_C) | Target_Z == 0) %>% 
  group_by(Cruise, Station, CampCN) %>% 
  fill(BactProd_C:BactAbund_sd, temp, Fluorescence, .direction = "up") %>% 
  drop_na(BactProd_C) %>% 
  ...
  left_join(., npp) %>% 
  select(Cruise:CampCN, Date, Latitude, Longitude, Z_MLD, Ez:Ez_NPP, Depth:sd_ba) %>% 
  arrange(CampCN) %>% 
  mutate(ba = ifelse(ba == "NaN", NA, ba),
         sd_ba = ifelse(sd_ba == "NaN", NA, sd_ba)) %>% 
  drop_na(Ez) %>% 
  group_by(Cruise, Station) %>% 
  mutate(ave_mld = mean(Z_MLD),
         sd_mld = sd(Z_MLD)) %>% 
  ungroup() %>% 
  select(Cruise:Z_MLD, ave_mld, sd_mld, everything())
...
to_interpolate.df <- copper %>% 
  select(Depth:ncol(.)) %>% 
  zoo(., order.by = .$Depth) 
interp_temp <- as.numeric(na.approx(to_interpolate.df$temp, na.rm = F))
interp_chl <- as.numeric(na.approx(to_interpolate.df$chl, na.rm = F))
...
interpolations.df <- data.frame(Depth, interp_temp, interp_chl, interp_bp, interp_ba)
}
...
int_bcd <- bcd %>% 
  group_by(CampCN) %>% 
  filter(Depth <= Ez) %>% 
  mutate(int.temp = integrateTrapezoid(Depth, interp_temp, type = "A"),
         int.chl = integrateTrapezoid(Depth, interp_chl, type = "A"),
         int.bcd = integrateTrapezoid(Depth, bcd, type = "A"),
...
# Integrated BCD
saveRDS(int_bcd, "~/GITHUB/naames_bioav_ms/Output/processed_BCD_4_2021.rds")
