...
export <- readRDS("~/GITHUB/naames_bioav_ms/Input/processed_export_for_bioavMS.5.29.20.rds") %>% 
  mutate(Cruise = gsub("AT39-6", "AT39", Cruise)) %>% 
  select(Cruise, Season, degree_bin, CampCN, Subregion, Station, Ez, Target_Z, interp_doc, interp_don, redis_DOC_vol, int_delta_DOC_ez, NCP_mol_ez) %>% 
  distinct() %>% 
  mutate_at(vars(redis_DOC_vol), round, 1) %>% 
  mutate_at(vars(int_delta_DOC_ez, NCP_mol_ez), round, 2) %>%
...
export_bioav <- full_join(readRDS("~/GITHUB/naames_bioav_ms/Output/processed_bge.rds") %>% 
                            ...
                            drop_na(interp_doc), 
                          export %>% 
                            select(Cruise, degree_bin, Subregion, Station, Ez, redis_DOC_vol, int_delta_DOC_ez, NCP_mol_ez) %>%  
                            group_by(Cruise, Station) %>% 
                            mutate(ave_Ez = mean(Ez),
                                   sd_Ez = sd(Ez),
                                   ave_int_delta_DOC_ez = mean(int_delta_DOC_ez, na.rm = T),
                            ...
                            select(-c(Ez, int_delta_DOC_ez, NCP_mol_ez)) %>% 
                            distinct()) %>% 
