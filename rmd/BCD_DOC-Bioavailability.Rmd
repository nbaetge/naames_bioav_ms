...
#ncp
ncp.boxplot <- doc %>% 
  select(Season,  ave_Ez, ave_NCP_mol_ez) %>%
  mutate_at(vars("ave_NCP_mol_ez"), funs(./ave_Ez * 10^3)) %>% 
  drop_na(ave_NCP_mol_ez) %>% 
  distinct() %>% 
  ggboxplot(., x = "Season", y = "ave_NCP_mol_ez", 
            order = c("Early Spring", "Late Spring", "Early Autumn", "Late Autumn"),
            xlab = F, 
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 7, fig.align = "center", warning = FALSE}

patchwork2 <- ncp.boxplot / deldoc.boxplot 
