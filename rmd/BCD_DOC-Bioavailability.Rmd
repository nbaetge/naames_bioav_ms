...
doc_doc.star.reg.plot <- vessel_comparisons %>% 
  ...
  theme(legend.title = element_blank()) +
  guides(fill = F) +
   annotate( geom = "text", label = expression(atop("y = 0.90x + 4.35", paste("r"^2,"= 0.83, ", ("p "), "<< 0.01"))), x = 78, y = 51, size = 3) +
  xlim(50, 80) +
  ylim(50, 80)
...
doc44a <- lat44 %>% 
  ...
  scale_color_manual(values = custom.colors) +
  scale_fill_manual(values = custom.colors) +
 labs(x = expression(""), y = expression(paste("DOC"[SA],", µmol C L"^-1))) +
  theme_classic2(base_size = 16) +
  theme(legend.title = element_blank()) +
...
doc44b <- lat44 %>% 
  ...
  scale_color_manual(values = custom.colors) +
  scale_fill_manual(values = custom.colors) +
 labs(x = expression(""), y = expression(paste("Bioavailable DOC"[SA],", µmol C L"^-1))) +
  theme_classic2(base_size = 16) +
  theme(legend.title = element_blank()) +
...
doc44c <- lat44 %>% 
  ...
  geom_errorbar(aes(ymin = ave_per.pers.doc - sd_per.pers.doc, ymax = ave_per.pers.doc + sd_per.pers.doc, color = factor(Season, levels = levels)), width = 1) +
  geom_point(aes(fill = factor(Season, levels = levels)), size = 3, shape = 21, alpha = 0.7 ) +
  geom_line(aes(color = Season, linetype = station_line), size = 0.75, alpha = 0.7) +
  scale_color_manual(values = custom.colors) +
  scale_fill_manual(values = custom.colors) +
 labs(x = expression("Days"), y = expression(paste("% DOC"[SA]," Remaining"))) +
  theme_classic2(base_size = 16) +
  theme(legend.title = element_blank()) +
  guides(color = F, linetype = F, fill = F)
...
reg1.plot <- bcd.data %>% 
  ...
  theme(legend.title = element_blank()) +
  guides(color = F) +
  annotate( geom = "text", label = expression(atop("y = 0.19x + 0.03", paste("r"^2," = 0.68, ", ("p "), "<< 0.01"))), x = 2.7, y = 0.08, size = 6) 
