...
my_comparisons2 <- list( c("Early Spring", "Late Spring"), c("Early Spring", "Early Autumn"),  c("Late Spring", "Early Autumn") ) 
...
ncp.boxplot <- export %>% 
  ...
  select(Season,  ave_Ez, NCP_mol_ez) %>%
  mutate_at(vars("NCP_mol_ez"), funs(./ave_Ez * 10^3)) %>% 
  drop_na(NCP_mol_ez) %>% 
  distinct() %>% 
  ggboxplot(., x = "Season", y = "NCP_mol_ez", 
            order = c("Early Spring", "Late Spring", "Early Autumn"),
            xlab = F, 
            ylab = expression(italic(paste("NCP, mmol C m"^-3))),
            add = "jitter",
            width = 0.5,
            ggtheme = custom_theme()) + 
  rremove("x.text") +
...
deldoc.boxplot <- export %>% 
  filter(!Season == "Late Autumn") %>% 
  select(Season, ave_Ez, int_delta_DOC_ez) %>% 
  mutate_at(vars(contains("int_delta_DOC")), funs(./ave_Ez * 10^3)) %>% 
  drop_na(int_delta_DOC_ez) %>% 
  distinct() %>% 
   ggboxplot(., x = "Season", y = "int_delta_DOC_ez", 
            order = c("Early Spring", "Late Spring", "Early Autumn"),
            xlab = F, 
...
#doc:ncp
doc_ncp.boxplot <- export %>% 
  filter(!Season == "Late Autumn") %>% 
  select(Season, doc_ncp_ez) %>%
  distinct() %>%
  drop_na(doc_ncp_ez) %>% 
  ggboxplot(., x = "Season", y = "doc_ncp_ez", 
            order = c("Early Spring", "Late Spring", "Early Autumn"),
            xlab = F, 
            ylab = expression(italic(paste("DOC:NCP"))),
            add = "jitter",
            width = 0.5,
            ggtheme = custom_theme()) + 
  rremove("x.text") +
...
#ncp_100
ncp_100.boxplot <- export %>% 
  filter(!Season == "Late Autumn") %>% 
  select(Season, NCP_mol_100) %>%
  distinct() %>% 
  mutate_at(vars("NCP_mol_100"), funs(./100 * 10^3)) %>% 
  drop_na(NCP_mol_100) %>% 
  distinct() %>% 
  ggboxplot(., x = "Season", y = "NCP_mol_100", 
            order = c("Early Spring", "Late Spring", "Early Autumn"),
            xlab = F, 
            ylab = expression(italic(paste("NCP, mmol C m"^-3))),
            add = "jitter",
...
#∆doc_100
deldoc_100.boxplot <- export %>% 
  filter(!Season == "Late Autumn") %>% 
  select(Season, int_delta_DOC_100) %>%
  distinct() %>% 
  mutate_at(vars("int_delta_DOC_100"), funs(./100 * 10^3)) %>% 
  drop_na(int_delta_DOC_100) %>% 
  distinct() %>% 
   ggboxplot(., x = "Season", y = "int_delta_DOC_100", 
            order = c("Early Spring", "Late Spring", "Early Autumn"),
            xlab = F, 
            ylab = expression(italic(paste("∆DOC, mmol C m"^-3))),
            add = "jitter",
...
#doc:ncp_100
doc_ncp_100.boxplot <- export %>% 
  filter(!Season == "Late Autumn") %>% 
  select(Season,  Station, doc_ncp_100) %>%
  drop_na(doc_ncp_100) %>% 
  distinct() %>% 
  ggboxplot(., x = "Season", y = "doc_ncp_100", 
            order = c("Early Spring", "Late Spring", "Early Autumn"),
            xlab = F, 
            ylab = expression(italic(paste("DOC:NCP"))),
            add = "jitter",
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 12, fig.width = 7, fig.align = "center", warning = FALSE}
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 12, fig.width = 6, fig.align = "center", warning = FALSE}

patchwork2 <- ncp_100.boxplot / deldoc_100.boxplot / doc_ncp_100.boxplot
...
patchwork3 <- ncp.boxplot / deldoc.boxplot / doc_ncp.boxplot
...
If experiments do this timepoint, they will be added and then interpolated. This timepoint is the longest timepoint shared by all all of the cruises. 
...
We've propogated the error for derived variables

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 16, fig.width = 8, fig.align = "center", warning = FALSE}
doc44a <- interpolated %>% 
  drop_na(norm_doc) %>% 
  group_by(Cruise, Station) %>%
  ggplot(aes(x = Days, y = norm_doc, group = interaction(Cruise, Station, degree_bin))) + 
...
doc44b <- interpolated %>% 
  group_by(Cruise, Station) %>%
  drop_na(norm_doc) %>% 
  mutate(per_doc = norm_doc/accm_doc * 100,
         per_doc_error = sqrt( (sd_doc/norm_doc)^2/(sd_accm_doc/accm_doc)^2 ) ) %>% 
  ggplot(aes(x = Days, y = per_doc, group = interaction(Cruise, Station, degree_bin))) + 
  geom_hline(aes(yintercept = 0)) +
  geom_vline(aes(xintercept = 7), linetype = 2) +
  geom_errorbar(aes(ymin = per_doc - per_doc_error, ymax = per_doc + per_doc_error, color = factor(Season, levels = levels)), width = 0.5) +
  geom_point(aes(fill = factor(Season, levels = levels)), size = 3, shape = 21, alpha = 0.7 ) +
  geom_line(aes(color = Season), size = 0.75, alpha = 0.7) +
  geom_label(aes(x = 7, y = 0, label = "7 days"), size = 4) +
  scale_color_manual(values = custom.colors) +
  scale_fill_manual(values = custom.colors) +
  #facet_grid(~factor(Season, levels = levels), scales = "free") +
...
doc44c <- interpolated %>%  
  ...
  drop_na(norm_doc) %>% 
  #filter(Days <= 68) %>% 
  mutate(delta_doc = norm_doc - last(norm_doc),
         last_doc_error = last(sd_doc),
         delta_doc_error = sqrt( (sd_doc)^2 + (last_doc_error)^2 )) %>% 
  ggplot(aes(x = Days, y = delta_doc, group = interaction(Cruise, Station, degree_bin))) + 
  geom_hline(aes(yintercept = 0)) +
  geom_vline(aes(xintercept = 7), linetype = 2) +
  geom_errorbar(aes(ymin = delta_doc - delta_doc_error, ymax = delta_doc + delta_doc_error, color = factor(Season, levels = levels)), width = 0.5) +
  geom_point(aes(fill = factor(Season, levels = levels)), size = 3, shape = 21, alpha = 0.7 ) +
  geom_line(aes(color = Season), size = 0.75, alpha = 0.7) +
  scale_color_manual(values = custom.colors) +
  scale_fill_manual(values = custom.colors) +
...
NPP, BP, and BCD are converted to: mmol C m^-3^ d^-1^
...
# ∆DOC
deldoc.bar <- export %>% 
  select(Cruise:Station, int_delta_DOC_100) %>%
  mutate(degree_bin = as.character(degree_bin)) %>% 
  mutate_at(vars(contains("int_delta_DOC")), funs(./100 * 10^3)) %>% 
  drop_na(int_delta_DOC_100) %>% 
  group_by(Season, Station, degree_bin) %>% 
  mutate(deldoc = mean(int_delta_DOC_100),
         sd_deldoc = sd(int_delta_DOC_100)) %>% 
  ungroup() %>% 
  select(Cruise:degree_bin, Station, deldoc, sd_deldoc) %>% 
  distinct() %>% 
  ##PLOT
  ggplot(aes(x = degree_bin, y =  deldoc, group = interaction(Season, Station, degree_bin)))  + 
  geom_bar(position = position_dodge(), stat = "identity", color = "black", fill = "#999999", alpha = 1) +
  geom_errorbar(aes(ymin = deldoc - sd_deldoc, ymax = deldoc + sd_deldoc), position = position_dodge(width = 0.9), stat = "identity", width = 0.1, size = 0.5) +
...
bcd.bar / bcd.npp.bar / ratio.bar / deldoc.bar +
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(size = 14))
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 4, fig.align = "center", warning = FALSE}
doc_star.data %>% 
  ggplot(aes(x = doc_star, y = doc, group = Bottle)) + 
  geom_abline(aes(intercept = 0, slope = 1)) +
  geom_abline(intercept = doc_star_bottle.reg$regression.results[3,2],
              slope = doc_star_bottle.reg$regression.results[3,3],colour = "black", linetype = 2, size = 1) +
  geom_point(aes(fill = factor(Season, levels = levels)), color = "black", shape = 21, size = 4, alpha = 0.7 ) +
  labs(x = expression(italic(paste("DOC*, µmol C L"^-1))), y = expression(italic(paste("DOC, µmol C L"^-1))),  caption = "Solid line represents the 1:1 line.") +
  scale_fill_manual(values = custom.colors) +
  custom_theme() +
   annotate( geom = "text", label = expression(atop("y = 1.06x - 2.36", paste("r"^2,"= 0.89, ", italic("p "), "<< 0.01"))), x = 75, y = 52, size = 3) +
  xlim(50, 80) +
  ylim(50, 80)
...
vl_btl_doc <- doc_pdoc.data %>% 
  ggplot(aes(x = doc, y = pdoc, group = Bottle)) + 
  geom_abline(aes(intercept = 0, slope = 1)) +
  geom_abline(intercept = doc_pdoc.data.reg$regression.results[3,2],
              slope = doc_pdoc.data.reg$regression.results[3,3],colour = "black", linetype = 2, size = 1) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 5, fig.align = "center", warning = FALSE}
vl_btl_cell <- btl_vial_cell.data %>% 
 ...
 geom_point(aes(fill = Station), color = "black", shape = 21, size = 4, alpha = 0.7 ) +
  labs(x = expression(italic(paste("Bottle Cells L"^-1))), y = expression(italic(paste("Vial Cells L"^-1))), fill = "Early Spring Station") +
  scale_y_continuous(breaks = pretty_breaks()) +
