...
```{r doc star plot, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 8}
vessel_comparisons %>% 
  drop_na(doc_star) %>% 
  ...
  labs(y = expression(italic(paste("DOC*, µmol C L"^-1))), x = expression(italic(paste("DOC, µmol C L"^-1))),  caption = "Solid line represents the identity line") +
...
```{r combine regression plots, fig.height=8, fig.width=16, message=FALSE, warning=FALSE}
...
doc44b <- lat44 %>% 
  ...
  ungroup() %>% 
  select(Season, Cruise, Station, Days, ave_del.bioav.doc, sd_del.bioav.doc) %>% distinct() %>% 
...
doc44c <- lat44 %>% 
  ...
  ungroup() %>% 
  select(Season, Cruise, Station, Days, ave_per.pers.doc, sd_per.pers.doc) %>% distinct() %>% 
...
```{r patchwork rate & abundance plots, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 12, fig.width = 7, fig.align = "center"}
