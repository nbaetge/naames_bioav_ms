...
saveRDS(figure1, "~/GITHUB/naames_bioav_ms/Output/map.rds")
...
doc44a_b <- lat44 %>% 
  mutate(norm.doc = ifelse(!Cruise == "AT39" & Days != 0 , NA, norm.doc),
         sd_combined_doc = ifelse(!Cruise == "AT39" & Days != 0 , NA,  sd_combined_doc)) %>% 
  ggplot(aes(x = Days, y = norm.doc, group = interaction(Cruise, Station, Bottle))) + 
  geom_hline(aes(yintercept = 0)) +
  geom_errorbar(aes(ymin = norm.doc - sd_combined_doc, ymax = norm.doc + sd_combined_doc, color = factor(Season, levels = levels)), width = 1) +
  geom_point(aes(fill = factor(Season, levels = levels)), size = 3, shape = 21, alpha = 0.7 ) +
...
doc44a_c <- lat44 %>% 
  mutate(norm.doc = ifelse(Cruise == "AT38" & Days != 0 , NA, norm.doc),
         sd_combined_doc = ifelse(Cruise == "AT38" & Days != 0 , NA,  sd_combined_doc)) %>% 
  ggplot(aes(x = Days, y = norm.doc, group = interaction(Cruise, Station, Bottle))) + 
  geom_hline(aes(yintercept = 0)) +
  geom_errorbar(aes(ymin = norm.doc - sd_combined_doc, ymax = norm.doc + sd_combined_doc, color = factor(Season, levels = levels)), width = 1) +
  geom_point(aes(fill = factor(Season, levels = levels)), size = 3, shape = 21, alpha = 0.7 ) +
...
npp.boxplot <- bcd %>% 
  ...
  scale_fill_viridis_d(option = "viridis") +
  theme_classic2(base_size = 16) +
  labs(x = "", y =  expression(paste("NPP, µmol C L"^-1, " d"^-1)), fill = "Latitude, ˚N") + 
...
bp.boxplot <- bcd %>% 
  ...
  scale_fill_viridis_d(option = "viridis") +
  theme_classic2(base_size = 16) +
  labs(x = "", y =  expression(paste("BP, µmol C L"^-1, " d"^-1)), fill = "Latitude, ˚N") + 
  guides(fill = F) +
  stat_compare_means(label.y = 0)  +
  stat_compare_means(comparisons = my_comparisons, 
                     label = "p.adj",
...
ba.boxplot <- bcd %>% 
  ...
  scale_fill_viridis_d(option = "viridis") +
  theme_classic2(base_size = 16) +
  labs(x = "", y =  expression(paste("BA, cells L"^-1)), fill = "Latitude, ˚N") +
  guides(fill = F) +
  stat_compare_means(label.y = 0)  +
  stat_compare_means(comparisons = my_comparisons, 
                     label = "p.adj",
...
bcd.bar <- bcd.summary %>% 
  ...
  labs(x = expression(""), y = expression(paste("BCD, µmol C L"^-1, " d"^-1)), colour = expression("")) +
  theme_classic2() +
...
bcd.npp.bar <- bcd.npp.summary  %>% 
  ...
  labs(x = expression(""), y = expression(paste("µmol C L"^-1, " d"^-1)), colour = expression("")) +
  scale_fill_manual(values = custom.colors) +
  theme_classic2() +
  facet_grid(~factor(Season, levels = levels), scales = "free") +
  theme(strip.background = element_blank(), strip.text = element_blank()) +
...
doc_sa.bar <-  export_bar %>%
  ...
  labs(x = expression(""), y = expression(paste("DOC"["SA"],", µmol C L"^-1)), colour = expression("")) +
  theme_classic2() +
  facet_grid(~factor(Season, levels = levels), scales = "free")
...
p1a <- ba_curves2 + labs(y = expression(paste("Log cells L"^-1))) +  theme_classic2(16)
...
p3 <- p3a / p3b / p3c
##
p4 <- map + theme_classic2(16) 
 
p5 <- doc44a_i + theme_classic2(16) + labs(fill = "")

p6 <- doc44a + theme_classic2(16) + labs(fill = "")

p7 <- doc44a_b + theme_classic2(16) + labs(fill = "")

p8 <- doc44a_c + theme_classic2(16) + labs(fill = "")

p9 <- (ba_curves2 + labs(y = expression(paste("Log cells L"^-1))) +  theme_classic2(16)) +
  (doc_curves2 + theme_classic2(16) + theme(strip.background = element_blank(), strip.text.x = element_blank())) +
  (bge_box + theme_classic2(18)) + plot_layout(nrow = 3, ncol = 1)
...
officer::read_pptx() %>%
  ...
  officer::add_slide() %>%
  officer::ph_with(p2, ph_location(width = 7, height = 4)) %>%
  
  officer::add_slide() %>%
  officer::ph_with(p3, ph_location(width = 10, height = 8)) %>%
  
  officer::add_slide() %>%
  officer::ph_with(p4, ph_location(width = 8, height = 7)) %>%
  
  officer::add_slide() %>%
  officer::ph_with(p5, ph_location(width = 10, height = 8)) %>%
  
  officer::add_slide() %>%
  officer::ph_with(p6, ph_location(width = 10, height = 8)) %>%
  
   officer::add_slide() %>%
  officer::ph_with(p7, ph_location(width = 10, height = 8)) %>%
  
   officer::add_slide() %>%
  officer::ph_with(p8, ph_location(width = 10, height = 8)) %>%
  
  officer::add_slide() %>%
  officer::ph_with(p9, ph_location(width = 9, height = 9)) %>%
...
ba_end <- read_csv("~/GITHUB/naames_bioav_ms/Input/master/N2-4_BactA_Remin_Master.csv") %>% 
  ...
  drop_na(cells)
```
```{r ba end, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}
ba_long <- ba_end %>% 
