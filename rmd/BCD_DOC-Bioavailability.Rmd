...
bcd_table <- bcd %>% 
  ...
  # ungroup() %>% 
  select(Cruise:degree_bin, int.bcd.season_bge) %>% 
  distinct() %>% 
  mutate_at(vars(int.bcd.season_bge), funs(./1000)) %>% 
  distinct() %>% 
  left_join(., bcd %>% 
  ...
  left_join(., bcd %>% 
              ...
              # ungroup()
              select(Cruise:Station, degree_bin, bcd.npp.season_bge) %>% 
              distinct()
            ) %>% 
  ...
  mutate(Cruise_ave_Ez = mean(ave_Ez),
         ...
         Cruise_ave_BCD = mean(int.bcd.season_bge),
         Cruise_sd_BCD = sd(int.bcd.season_bge),
         Cruise_ave_BCD_NPP = mean(bcd.npp.season_bge),
         Cruise_sd_BCD_NPP = sd(bcd.npp.season_bge)
         ) %>% 
  ungroup() %>% 
  group_by(Cruise, Subregion, degree_bin) %>% 
  mutate(Bin_ave_NPP = mean(int.NPP),
         Bin_sd_NPP = mean(int.NPP),
         Bin_ave_BCD = mean(int.bcd.season_bge),
         Bin_sd_BCD = sd(int.bcd.season_bge),
         Bin_ave_BCD_NPP = mean(bcd.npp.season_bge),
         Bin_sd_BCD_NPP = sd(bcd.npp.season_bge)
         ) %>% 
  ungroup() %>% 
  mutate_at(vars(contains("BCD_NPP")), round) %>% 
  mutate_at(vars(Cruise_ave_NPP:Cruise_sd_BCD, Bin_ave_NPP:Bin_sd_BCD ), round, 2) %>% 
...
Seasonally accumulated DOC is expressed as mmol C m^-3^

```{r}
bioav.table <- doc %>% 
  select(Season:Subregion, degree_bin, Ez, int_delta_DOC_ez, contains("bioav_accm_doc"), contains("bioav_doc"), contains("per_bioav"), contains("persis"), contains("ddoc")) %>% 
  distinct() %>% 
  group_by(Season, Subregion, degree_bin) %>% 
  mutate(int_delta_DOC_ez = int_delta_DOC_ez/Ez * 10^3) %>% 
...
bcd_bioav <- bcd %>% 
  ...
  select(Cruise:degree_bin, int.bcd.season_bge) %>% distinct() %>% 
  left_join(., bcd %>% 
  ...
  left_join(., bcd %>% 
              ...
              # ungroup()
              select(Cruise:degree_bin, bcd.npp.season_bge) %>% distinct()
  ) %>% 
  left_join(bcd %>% 
  ...
  mutate(int_delta_DOC_ez_mM = round(int_delta_DOC_ez/ave_Ez * 10^3, 2))
```
...
```{r echo = FALSE}
reg2 <- lmodel2(ave_BCD ~ int_delta_DOC_ez_mM, data = bcd_bioav, nperm = 99)
```
...
reg2.plot  <- bcd_bioav %>% 
  ggplot(aes(x = int_delta_DOC_ez_mM, y = ave_BCD)) + 
  geom_abline(intercept = reg2$regression.results[3,2],
              slope = reg2$regression.results[3,3],colour = "black", linetype = 2, size = 1) +
  geom_errorbar(aes(color = factor(Season, levels = levels), ymin = ave_BCD - sd_BCD, ymax = ave_BCD + sd_BCD), width = 0.02) +
  geom_point( aes(fill = factor(Season, levels = levels)), shape = 21, color = "black", size = 4, alpha = 1) +
  ...
  annotate( geom = "text", label = expression(atop("y = 0.46x + 0.08", paste("r"^2," = 0.01, ", italic("p "), "= 0.61"))), x = 7, y = 0.7, size = 4) 
