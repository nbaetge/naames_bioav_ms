...
- cell_resp = ∆POC/∆TOC (N3 & 4) or TOC_star (N2)
...
#######toc and doc star###
  mutate(toc.star_i = ifelse(Cruise == "AT34" & Hours == 0, round(doc + i.poc, 1), NA),
         toc.star_s = ifelse(Cruise == "AT34" & Hours == stationary.harvest, round(interp_doc + s.poc, 1), NA),
         doc.star_i = ifelse(Cruise != "AT34" & Hours == 0, round(ptoc - i.poc, 1), NA),
...
######delta toc, toc.star, doc, doc.star####
  mutate(del.toc = ifelse(Cruise != "AT34" & Hours == stationary.harvest, first(ptoc) - interp_ptoc, NA),
         ...
         del.toc.star = toc.star_i - toc.star_s,
         del.toc.star = ifelse(del.toc.star < 1.75, NA, del.toc.star),
...
mutate(bge = ifelse(Cruise == "AT34", del.poc/del.doc, del.poc/del.doc.star),
  ...
  ungroup() %>% 
  ######cell sp resp fmol c L-1####
  mutate(cell_resp = ifelse(Cruise == "AT34", round((del.toc.star / del.cells) * 10^9, 1), round((del.toc / del.cells) * 10^9, 1)),
         cell_resp = ifelse(Cruise == "AT34" & Station == 3, NA, cell_resp)) %>% 
...
bge_summary <- calcs %>% 
  ...
  select(Season:Treatment, contains("del"), bge, cell_resp) %>% 
  distinct() 

bge_cruise_means <- bge_summary %>% 
 group_by(Cruise, Season) %>% 
  summarise_at(vars(del.cells:cell_resp), list(mean = mean, sd = sd), na.rm = T) %>% 
  arrange(factor(Season, levels = c("Early Spring", "Late Spring", "Early Autumn")))
