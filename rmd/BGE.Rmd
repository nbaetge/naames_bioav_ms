...
input %>% 
  drop_na(delta_doc) %>% 
  ggplot(aes(x = Days, y = delta_doc)) + 
  # geom_errorbar(aes(ymin = deltadoc - sd_doc, ymax = doc + sd_doc, color = Bottle), width = 0.5) +
  ...
  geom_abline(intercept = model2$coefficients[[1]],
              slope = model2$coefficients[[2]],colour = "black", linetype = 2, size = 1) +
  geom_vline(aes(xintercept = stationary/24), linetype = 3, size = 1, color = "blue") +
  scale_color_manual(values = custom.colors) +
  scale_fill_manual(values = custom.colors) +
  labs(y = expression(paste("∆DOC, µmol C L"^-1))) +
  custom_theme() +
  guides(fill = F, color = F) +
  annotate( geom = "text", label = expression( paste("slope = -0.09")), x = 25, y = 0, size = 6) +
  
  
input %>% 
  drop_na(delta_bc) %>% 
  ggplot(aes(x = Days, y = delta_bc)) + 
  # geom_errorbar(aes(ymin = deltadoc - sd_doc, ymax = doc + sd_doc, color = Bottle), width = 0.5) +
  geom_point(aes(fill = Bottle), size = 4, shape = 21, alpha = 0.7 ) +
  geom_line(aes(color = Bottle), alpha = 0.7) +
  geom_abline(intercept = model$coefficients[[1]],
              slope = model$coefficients[[2]],colour = "black", linetype = 2, size = 1) +
  geom_vline(aes(xintercept = stationary/24), linetype = 3, size = 1, color = "blue") +
  scale_color_manual(values = custom.colors) +
  scale_fill_manual(values = custom.colors) +
  labs(y = expression(paste("Bacterial Carbon, µmol C L"^-1))) +
  custom_theme() +
  guides(fill = F, color = F) +
  annotate( geom = "text", label = expression( paste("slope = 0.4")), x = 0.5, y = 1.5, size = 6) +
  
  plot_annotation(title = "AT39 S2RD") 
```
