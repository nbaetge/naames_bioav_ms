...
bge_summary %>%
  ...
  mutate(n = paste("n = ", n)) %>% 
 ##PLOT
  ggplot(aes(x = factor(group, levels = c("GF75 POC", "NAAMES CCF", "Fukuda CCF", "Lee CCF" )), y = group_bge)) +
   ...
   scale_y_continuous(breaks = pretty_breaks()) +
    scale_fill_manual(values = custom.colors) +
    labs(x = expression(italic("")), y = expression(italic(paste("BGE"))), colour = expression(italic(""))) +
    custom_theme() +
...
bge_summary %>%
  ...
  ungroup() %>% 
  select(Season, group, group2, group_bge:med_group_bge, n, group2_bge:med_group2_bge) %>% 
  distinct() %>% 
  mutate_at(vars(group_bge:med_group_bge, group2_bge:med_group2_bge), round, 2)
...
int_bcd %>% 
  ...
  labs(x = expression(italic("Latitude, ˚N")), y = expression(italic(paste("BCD, mmol C m"^-3, "d"^-1))), colour = expression(italic(""))) +
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
  facet_grid(~factor(Season, levels = levels), scales = "free") 
```
...
int_bcd %>% 
  ...
  geom_errorbar(aes(ymin = ave - sd, ymax = ave + sd), position = position_dodge(width = 0.9), stat = "identity", width = 0.1, size = 0.5) +
  labs(x = expression(italic("Latitude, ˚N")), y = expression(italic(paste("mmol C m"^-3, "d"^-1))), colour = expression(italic(""))) +
  scale_y_continuous(breaks = pretty_breaks()) +
  #scale_fill_brewer(palette = "Set1") +
  scale_fill_manual(values = custom.colors) +
  custom_theme() +
...
int_bcd %>% 
  select(Cruise:Station, degree_bin, ave_Ez, int.ba.ez, int.bp.ez, sd_int.bp.ez, sd_int.ba.ez) %>%
  distinct() %>% 
  pivot_longer(cols = c(int.ba.ez, int.bp.ez), names_to = "var", values_to = "value") %>% 
  mutate(var = ifelse(var == "int.ba.ez", "Abundance", "Production"),
         degree_bin = as.character(degree_bin)) %>% 
  group_by(Cruise, Station, var) %>% 
  mutate(sd = ifelse(var == "Abundance", sd_int.ba.ez, sd_int.bp.ez)) %>% 
  ##PLOT
  ggplot(aes(x = degree_bin, y = value), group = interaction(Season, Station, degree_bin))  +
  geom_bar( position = position_dodge2(), stat = "identity", color = "black", alpha = 1, fill = "#999999") +
  geom_errorbar(aes(ymin = value - sd, ymax = value + sd), position = position_dodge(width = 0.9), stat = "identity", width = 0.1, size = 0.5) +
  labs(x = expression(italic("Latitude, ˚N")), y = expression(italic(paste("µmol C m"^-3, "d"^-1, "                     ", "Cells m"^-3))), colour = expression(italic(""))) +
  #hahahaha this ain't pretty but it works ^
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
  facet_grid(var~factor(Season, levels = levels), 
             scales = "free", 
             switch = "y") 
```

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 12, fig.align = "center", warning = FALSE}
int_bcd %>% 
  select(Cruise:Station, degree_bin, ave_Ez, int.ba.ez, int.bp.ez, sd_int.bp.ez, sd_int.ba.ez) %>%
  distinct() %>% 
  pivot_longer(cols = c(int.ba.ez, int.bp.ez), names_to = "var", values_to = "value") %>% 
  ...
  ##PLOT
  ggplot(aes(x = degree_bin, y = value, group = interaction(Season, Station, degree_bin)))  +
  geom_bar( position = position_dodge(width = 0.9), stat = "identity", color = "black", alpha = 1, fill = "#999999") +
  geom_errorbar(aes(ymin = value - sd, ymax = value + sd), position = position_dodge(width = 0.9), stat = "identity", width = 0.1, size = 0.5) +
  labs(x = expression(italic("Latitude, ˚N")), y = expression(italic(paste( "Bacterial Abundance, Cells m"^-3))), colour = expression(italic(""))) +
  #hahahaha this ain't pretty but it works ^
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
...
int_bcd %>% 
  ...
  geom_bar(position = position_dodge(width = 0.9), stat = "identity", color = "black", alpha = 1, fill = "#999999") +
  geom_errorbar(aes(ymin = value - sd, ymax = value + sd), position = position_dodge(width = 0.9), stat = "identity", width = 0.1, size = 0.5) +
  labs(x = expression(italic("Latitude, ˚N")), y = expression(italic(paste("Bacterial Production, µmol C m"^-3, "d"^-1))), colour = expression(italic(""))) +
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
  facet_grid(~factor(Season, levels = levels), 
             scales = "free") #+
