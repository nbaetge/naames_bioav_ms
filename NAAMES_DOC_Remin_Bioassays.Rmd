...
We'll estimate BCD for each station of all the cruises using bacterial production and the global average BGE of 22%. Not that we'll be averaging casts taken throughout the day/night. 

```{r}
bcd <- read_rds("Input/processed_bf.2.2020.rds") %>% 
  ...
  ungroup() %>% 
  select(Cruise:Depth, max_depth, everything()) %>% 
  group_by(Cruise, Station, CampCN, Depth) %>% 
  mutate(bge = 0.22,
         cruise_bge 
         bcd = bp/bge,
         ) %>% 
  mutate_at(vars(bcd), round) %>% 
  ungroup() %>% 
  group_by(Cruise, Station, Depth) %>% 
  mutate(bcd.station = mean(bcd) * 10^3,
         sd_bcd.station = sd(bcd) * 10^3) %>% 
  mutate_at(vars(bcd.station, sd_bcd.station), round) %>% 
  ungroup() %>% 
  select(Cruise:degree_bin, Depth, bcd.station, sd_bcd.station) %>% 
  distinct() %>% 
  arrange(Cruise, Station, Depth) %>% 
  group_by(Cruise, Station) %>% 
  filter(Depth <= 500) %>% 
  mutate(int_bcd_500 = integrateTrapezoid(Depth, bcd.station, type = "A")) %>% 
  filter(Depth <= 300) %>% 
  mutate(int_bcd_300 = integrateTrapezoid(Depth, bcd.station, type = "A")) %>% 
  filter(Depth <= 100) %>% 
  mutate(int_bcd_100 = integrateTrapezoid(Depth, bcd.station, type = "A"),
         int_bcd_100_300 = int_bcd_300 - int_bcd_100) %>% 
  mutate_at(vars(contains("int_bcd")), round)
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10, fig.align = "center", warning = FALSE}
bcd %>% 
  select(Cruise:Station, degree_bin, contains("int_bcd"))  %>% 
  distinct() %>% 
  ##PLOT
  ggplot(aes(x = factor(Season, levels = levels), y = (int_bcd_100/10^3) * (12/10^3) ))  +
  geom_boxplot(width = 0.2) +
  geom_jitter(width = 0.1, shape = 21, size = 3, alpha = 0.7) +
  labs(x = "", y = expression(italic(paste("BCD, µmol C m"^-2, "d"^-1))), colour = expression(italic(""))) +
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
  ggtitle("Bacterial Carbon Demand, Surface 100 m")
#facet_grid(type~factor(Season, levels = levels) , scales = "free") +
  #ggtitle("Delta POC") 
```
