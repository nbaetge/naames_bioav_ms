...
bactcarbon %>% 
  ...
  select(Bottle, bactC_raw, bactN_raw) %>% 
  gather(key, value, bactC_raw, bactN_raw) %>% 
  mutate(Bottle = gsub("100mls", "0.10 L", Bottle),
         ...
         key = gsub("bactC_raw", "Carbon", key),
         key = gsub("bactN_raw", "Nitrogen", key)) %>% 
  arrange(key, Bottle) %>% 
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10, fig.align = "center", warning = FALSE}
fg_cell %>% 
  select(Season, Station, Depth, Treatment, init_gf75_retention, stat_gf75_retention) %>% 
  mutate(facet_depth = paste(Depth, "m")) %>% 
  gather(key, value, init_gf75_retention:stat_gf75_retention) %>% 
  mutate(key = gsub("init_gf75_retention", "Initial", key),
         key = gsub("stat_gf75_retention", "Stationary", key)) %>% 
...
As a result, we will replace the CCF and NCF values of those samples with the following criteria:
...
- If samples were taken from the stationary phase, values will be replaced with those taken from the replicate sample from the same treatment if possible. There are 3 sets of experiments that do not have conversion factors for the stationary phase of either replicate bottle: AT34 S3 200 m, AT38 S1 MixDS, AT38 S1 MixSD.
  ...
  + We'll have to remain aware that caclulating CCFs for AT38 may not be appropriate
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10, fig.align = "center", warning = FALSE}
fg_cell.rm %>% 
  select(Season, Station, Depth, Treatment, init_ccf, stat_ccf) %>% 
  mutate(facet_depth = paste(Depth, "m"),
         Manipulation = ifelse(Treatment == "Control", "Control", "Non-Control")) %>% 
  gather(key, value, init_ccf:stat_ccf) %>% 
  mutate(key = gsub("init_ccf", "Initial", key),
         key = gsub("stat_ccf", "Stationary", key)) %>% 
  ggplot(aes(x = Season, y = value)) + 
  geom_boxplot(width = 0.3) +
  geom_jitter(aes(fill = key), width = 0.1, shape = 21, size = 6, alpha = 0.7) + 
  ...
  scale_fill_brewer(palette = "Set1") +
  custom_theme() +
  facet_grid(Manipulation~facet_depth) + 
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10, fig.align = "center", warning = FALSE}
fg_cell.rm %>% 
  select(Season, Station, Depth, Treatment, init_bactC_N_raw, stat_bactC_N_raw) %>% 
  mutate(facet_depth = paste(Depth, "m"),
         Manipulation = ifelse(Treatment == "Control", "Control", "Non-Control")) %>% 
  gather(key, value, init_bactC_N_raw:stat_bactC_N_raw) %>% 
  mutate(key = gsub("init_bactC_N_raw", "Initial", key),
         key = gsub("stat_bactC_N_raw", "Stationary", key)) %>% 
  ggplot(aes(x = Season, y = value)) + 
  geom_boxplot(width = 0.3) +
  geom_jitter(aes(fill = key), width = 0.1, shape = 21, size = 6, alpha = 0.7) + 
...
```{r echo = FALSE, results = 'asis'}
fg_cell.rm %>% 
  select(init_bactC_N_corr) %>% 
  summary()
```

```{r echo = FALSE, results = 'asis'}
fg_cell.rm %>% 
  select(stat_bactC_N_raw) %>% 
  summary()
```
...
Overall (including AT38), the mean bacterial C:N ratios for the in situ community (initial condition) estimated here are higher than those reported for oceanic bacteria by Fukuda et al. (1998): $$6.8 \pm 1.2$$ 
...
ggplot(aes(x =  init_biovol, y = init_ccf)) +
  geom_point(aes(fill = Season), shape = 21, color = "black", size = 4, alpha = 0.7 ) +
...
For the samples in which we have concurrent biovolume and cellular C measurements, we see that their relationship to one another is pretty widespread relative to published literature relationshops. 
...
cellC_plot.data %>% 
  ggplot(aes(x =  bactC_raw, y = ccf)) +
  geom_point( aes(color = Season, shape = phase), size = 4, alpha = 0.7 ) +
  labs(x = expression(paste("Bacterial POC on GF75 Filter (raw), umol C L"^-1)), y = expression(paste("Cellular C, fg C Cell"^-1))) +
  scale_x_continuous(trans = 'log10', breaks = pretty_breaks()) +
  scale_y_continuous(trans = 'log10', breaks = pretty_breaks(), limits = c(1, 90)) +
  scale_color_manual(values = custom.colors) +
  custom_theme_linedraw()  +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 13, fig.align = "center", warning = FALSE}
cellC_plot.data %>% 
  ggplot(aes(x =  bactC_corr, y = ccf)) +
  geom_point( aes(color = Season, shape = phase), size = 4, alpha = 0.7 ) +
  labs(x = expression(paste("Bacterial POC on GF75 Filter (corrected), umol C L"^-1)), y = expression(paste("Cellular C, fg C Cell"^-1))) +
  scale_x_continuous(trans = 'log10', breaks = pretty_breaks()) +
  scale_y_continuous(trans = 'log10', breaks = pretty_breaks(), limits = c(1, 90)) +
  scale_color_manual(values = custom.colors) +
  custom_theme_linedraw()  +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 13, fig.align = "center", warning = FALSE}
cellC_plot.data %>% 
  ggplot(aes(x =  bactC_N_raw, y = ccf)) +
  geom_point( aes(color = Season, shape = phase), size = 4, alpha = 0.7 ) +
  labs(x = expression(paste("Bacterial C:N (raw), molar")), y = expression(paste("Cellular C, fg C Cell"^-1))) +
  scale_x_continuous(trans = 'log10', breaks = pretty_breaks()) +
  scale_y_continuous(trans = 'log10', breaks = pretty_breaks(), limits = c(1, 90)) +
  scale_color_manual(values = custom.colors) +
  custom_theme_linedraw()  +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 13, fig.align = "center", warning = FALSE}
cellC_plot.data %>% 
  ggplot(aes(x =  bactC_N_corr, y = ccf)) +
  geom_point( aes(color = Season, shape = phase), size = 4, alpha = 0.7 ) +
  labs(x = expression(paste("Bacterial C:N (corrected), molar")), y = expression(paste("Cellular C, fg C Cell"^-1))) +
  scale_x_continuous(trans = 'log10', breaks = pretty_breaks()) +
  scale_y_continuous(trans = 'log10', breaks = pretty_breaks(), limits = c(1, 90)) +
  scale_color_manual(values = custom.colors) +
  custom_theme_linedraw()  +
...
We need to readjust our initial bacterial carbon numbers before calculating other parameters. Since the initial condition was 1.2-µm filtrate, the carbon content of that filtrate would be higher than the 30:70 incubation mix at the beginning of the experiment. As a result we'll correct the initial carbon number to account for this: POC~initial~ = POC~1.2 µm filtrate~ - 0.3(POC~1.2 µm filtrate~)
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 14, fig.align = "center", warning = FALSE}
bge_approach_lvls <- c("Point to Point, POC (raw)", "Point to Point, POC (corr)", "Point to Point, CCF", "Phase to Phase, CCF", "Area Under the Curve, CCF" )

bge_summary %>%
  select(Season:Bottle, contains("bge")) %>% 
  distinct() %>% 
  gather(Approach, BGE, bge_p_ccf, bge_ph, bge_ac, bge_GF75_raw, bge_GF75_corr, -c(Season:Bottle)) %>% 
  drop_na(BGE) %>% 
  mutate(Approach = gsub("bge_ph", "Phase to Phase, CCF", Approach),
         Approach = gsub("bge_p_ccf", "Point to Point, CCF", Approach),
         Approach = gsub("bge_ac", "Area Under the Curve, CCF", Approach),
         Approach = gsub("bge_GF75_raw", "Point to Point, POC (raw)", Approach),
         Approach = gsub("bge_GF75_corr", "Point to Point, POC (corr)", Approach),
         Depth = as.character(Depth),
         Depth = gsub("10", "10 m",  Depth),
         Depth = gsub("200", "200 m", Depth)) %>%
  ggplot(aes(x = factor(Approach, levels = bge_approach_lvls ), y = BGE)) +
  geom_boxplot(width = 0.3) +
  geom_jitter(aes(fill = factor(Approach, levels = bge_approach_lvls )), width = 0.1, shape = 21, size = 6, alpha = 0.7) + 
  labs(x = "", y = expression(italic("BGE")), colour = expression(italic(""))) + 
  scale_fill_brewer(palette = "Set1") +
  scale_y_continuous(breaks = pretty_breaks(), limits = c(-0.5, 1)) +
  custom_theme() +
  facet_grid(Manipulation~Season, scales = "free") +
...
Despite all this... BGEs estimated using CCFs from GF75 POC are trash. Note that in the figure above, I've limited the data to BGEs lower than 1. There are several BGEs estimated using CCF that are above 1. Using just uncorrected and corrected POC values (not applying bacterial abundances to them) dampens the BGEs to those closer to reality. However, there are several BGEs that are negative. This is because the POC value at stationary is lower than the initial condition, which is weird...but the differences may be within the limit of detection?
...
Because the surface and deepwater CCFs calculated for AT38 are high relative to the surface values of the other cruises, they are suspicious. Furthermore, they are blank corrected using the blanks from the spring cruises, which may not be appropriate. We may need to omit the AT38 data all together. 
...
mean_ccf.table <- mean_ccf %>% 
  select(Depth, Addition:sd_stat_ccf) %>%
  distinct() %>% 
  arrange(Depth)
```
