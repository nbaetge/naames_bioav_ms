...
```{r echo = F}
filt.data <- t.test.data %>% 
  filter(Filter == 1, Filtrate %in% c("0.2 µm", "1.2 µm")) 
...
```{r echo = F}
filt.data <- t.test.data %>% 
   filter(Filter == 2, Filtrate %in% c("0.2 µm", "1.2 µm")) 
...
```{r echo = F}
filt.data <- t.test.data %>% 
  filter(Filter == 2, Filtrate %in% c("0.2 µm", "1.2 µm"), !Season == "Early Autumn") 
...
bge_summary %>%
  ...
  scale_fill_brewer(palette = "Set1") +
  scale_y_continuous(breaks = pretty_breaks(), limits = c(0, 1.5)) +
  custom_theme() +
...
Despite all this... BGEs estimated using CCFs from GF75 POC are trash. Note that in the figure above, I've limited the data to BGEs lower than 1. There are several BGEs estimated using CCF that are above 1.5. Using just uncorrected and corrected POC values (not applying bacterial abundances to them) dampens the BGEs to those closer to reality. However, there are a few BGEs that are above 1, all in the early autumn cruise.
