...
int_bcd %>% 
  ...
  labs(x = expression(italic("")), y = expression(italic(paste( "Bacterial Abundance, Cells m"^-3))), colour = expression(italic(""))) +
...
export_bioav %>% 
  ...
  geom_errorbar(aes(ymin = norm_doc - trt_sd_doc, ymax = norm_doc + trt_sd_doc), width = 0.5) +
  geom_point(aes(fill = Season), size = 3, shape = 21, alpha = 0.7 ) +
...
bcd_bioav_table %>% 
  ...
  scale_fill_manual(values = custom.colors) +
  custom_theme() #+
   # annotate( geom = "text", label = expression(atop("y = 0.16x + 0.08", paste("r"^2,"= 0.66, ", italic("p "), "<< 0.01"))), x = 2.5, y = 0.1, size = 7) 
...
bcd_bioav_table %>% 
  ...
  #            slope = bcd_doc.reg$regression.results[3,3],colour = "black", linetype = 2, size = 1) + 
  geom_point( aes(fill = factor(Season, levels = levels)), shape = 21, size = 4, alpha = 0.7 ) +
  labs(x = expression(italic(paste("DOC:DON"))), y = expression(italic(paste("BCD, µmol C m"^-2, "d"^-1)))) +
  scale_fill_manual(values = custom.colors) +
  custom_theme() #+
   # annotate( geom = "text", label = expression(atop("y = 0.16x + 0.08", paste("r"^2,"= 0.66, ", italic("p "), "<< 0.01"))), x = 2.5, y = 0.1, size = 7) 
