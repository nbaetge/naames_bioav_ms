...
bcd %>% 
  ...
  ##PLOT
  ggplot(aes(x = degree_bin, fill = bge_used, y =  bcd.npp, group = interaction(Season, Station, degree_bin)))  +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 3, fig.width = 7, fig.align = "center", warning = FALSE}
bcd %>% 
  pivot_longer(cols = c(bcd.ez_npp.cr_bge, bcd.ez_npp), names_to = "bge_used", values_to = "bcd.npp") %>% 
  mutate(bge_used = ifelse(bge_used == "bcd.ez_npp.cr_bge", "Cruise Specific BGE", "Global BGE"),
         degree_bin = as.character(degree_bin)) %>% 
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 12, fig.align = "center", warning = FALSE}
bcd %>% 
  select(Cruise:Station, degree_bin, ave_Ez, int.ba.ez) %>% 
  mutate(degree_bin = as.character(degree_bin)) %>% 
  ##PLOT
  ggplot(aes(x = degree_bin, y =  int.ba.ez/ave_Ez, group = interaction(Season, Station, degree_bin)))  +
  geom_bar(position = position_dodge(), stat = "identity", color = "black", alpha = 1) +
  labs(x = expression(italic("Latitude, ˚N")), y = expression(italic(paste("Cells m"^-3))), colour = expression(italic(""))) +
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
  facet_grid(~factor(Season, levels = levels), scales = "free")
```
...
seas_bioav.table <- export_bioav %>% 
  filter(Treatment == "Control", Depth == "10") %>% 
  select(Season:Station, total.bioav_doc:per_persis, st.ddoc, lt.ddoc) %>% 
  distinct() %>% 
  group_by(Season) %>% 
  summarize(min_st.bioav_doc = min(st.bioav_doc, na.rm = T),
            mean_st.bioav_doc = mean(st.bioav_doc, na.rm = T),
            med_st.bioav_doc = median(st.bioav_doc, na.rm = T),
            max_st.bioav_doc = max(st.bioav_doc, na.rm = T),
            
            min_lt.bioav_doc = min(lt.bioav_doc, na.rm = T),
            mean_lt.bioav_doc = mean(lt.bioav_doc, na.rm = T),
            med_lt.bioav_doc = median(lt.bioav_doc, na.rm = T),
            max_lt.bioav_doc = max(lt.bioav_doc, na.rm = T),
            
            min_total.bioav_doc = min(total.bioav_doc, na.rm = T),
            mean_total.bioav_doc = mean(total.bioav_doc, na.rm = T),
            med_total.bioav_doc = median(total.bioav_doc, na.rm = T),
            max_total.bioav_doc = max(total.bioav_doc, na.rm = T),
            ...
            min_st.ddoc = min(st.ddoc, na.rm = T),
            mean_st.ddoc = mean(st.ddoc, na.rm = T),
            med_st.ddoc = median(st.ddoc, na.rm = T),
            max_st.ddoc = max(st.ddoc, na.rm = T),
            
            min_lt.ddoc = min(lt.ddoc, na.rm = T),
            mean_lt.ddoc = mean(lt.ddoc, na.rm = T),
            med_lt.ddoc = median(lt.ddoc, na.rm = T),
            max_lt.ddoc = max(lt.ddoc, na.rm = T),
            
            min_st.per_bioav = min(st.per_bioav, na.rm = T),
            mean_st.per_bioav = mean(st.per_bioav, na.rm = T),
            med_st.per_bioav = median(st.per_bioav, na.rm = T),
            max_st.per_bioav = max(st.per_bioav, na.rm = T),
            
            min_lt.per_bioav = min(lt.per_bioav, na.rm = T),
            mean_lt.per_bioav = mean(lt.per_bioav, na.rm = T),
            med_lt.per_bioav = median(lt.per_bioav, na.rm = T),
            max_lt.per_bioav = max(lt.per_bioav, na.rm = T),
            
            min_total.per_bioav = min(total.per_bioav, na.rm = T),
            mean_total.per_bioav = mean(total.per_bioav, na.rm = T),
            med_total.per_bioav = median(total.per_bioav, na.rm = T),
            max_total.per_bioav = max(total.per_bioav, na.rm = T),
  ...
  mutate_at(vars(min_st.bioav_doc:max_persis_doc), round, 1) %>% 
  mutate_at(vars(min_st.ddoc:max_persis), round) 
```
