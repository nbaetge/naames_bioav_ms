...
Based on these results, it seems like filter 2 of the blanks and the initial conditions could be a good blank. 
...
and the mean of filter 2 of only TFF blanks are the same. So, we'll calculate 2 corrected values:
...
We'll calculate 4 CCFs (fg C cell^-1^) from the POC data:
...
fg_cell %>% 
  ...
  custom_theme() +
  facet_grid(facet_depth~key, scales = "free")
...
fg_cell.qc %>% 
  filter(!Station == "U") %>% 
  select(Season, Station, Depth, Bottle, Treatment, gf75.flag, i.ccf.c1, i.ccf.c3, s.ccf.c1, s.ccf.c3) %>% 
  mutate(facet_depth = paste(Depth, "m"),
  ...
  mutate(label = key,
         label = gsub("i.ccf.c1", "POC corr. filt 2, all or only TFF blanks", label),
         label = gsub("i.ccf.c3", "POC corr. filter 1 + 2, TFF blanks", label),
         label = gsub("s.ccf.c1", "POC corr. filt 2, all or only TFF blanks", label),
         label = gsub("s.ccf.c3", "POC corr. filter 1 + 2, TFF blanks", label),
         time = ifelse(key %in% c("i.ccf.c1", "i.ccf.c3"), "Initial", "Stationary"),
         value = ifelse(time == "Initial" & Bottle %in% c("B", "D", "F"), NA, value)) %>% 
  ggplot(aes(x = Season, y = value)) + 
  geom_boxplot(width = 0.3) +
  geom_jitter(aes(color = time, shape = gf75.flag), width = 0.1,  size = 4, alpha = 0.6) + 
  labs(x = "", y = expression(italic("CCF, fg C cell"^-1)), colour = expression(italic(""))) + 
  scale_color_brewer(palette = "Set1") +
  #scale_shape_manual(values = c("OK" = 16, "Initial Questionable" = 2, "Stationary Questionable" = 3)) +
  custom_theme() +
  facet_grid(facet_depth~type+label, scales = "free") + 
  ggtitle("Carbon Conversion Factors")

```

There's a lot going on this plot. "Questionable" refers to samples with retentions >= 56%, where cell slippage may have occured. "ND" refers to points where there is missing filter retention data. We've faceted the plot by (1) control or non-control, (2) the type of blank correction, and (3) depth. 

Overall, there are mostly questionable measurements at 200 m. It is peculiar that the acceptable measurements at that depth horizon have CCFs >20. There are a lot of acceptable measurments at 10 m, with outliers at the initial condition being questionalble measurements. The CCFs for the Early Autumn still seem high even excluding the outliers. 

The POC correction using both filters of the TFF blanks appears to dampen the CCFs. 

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 22, fig.align = "center", warning = FALSE}
fg_cell.qc %>% 
  filter(!Station == "U") %>% 
  select(Season, Station, Depth, Bottle, Treatment, gf75.flag, i.cn, i.cn.c1, i.cn.c3, s.cn, s.cn.c1, s.cn.c3) %>% 
  mutate(facet_depth = paste(Depth, "m"),
         type = ifelse(Treatment == "Control", "Control", "Non-Control"),
  ...
  mutate(label = key,
         label = gsub("i.cn.c1", "POC corr. filt 2, all or only TFF blanks", label),
         label = gsub("i.cn.c3", "POC corr. filter 1 + 2, TFF blanks", label),
         label = gsub("i.cn", "POC uncorr.", label),
          label = gsub("s.cn.c1", "POC corr. filt 2, all or only TFF blanks", label),
         label = gsub("s.cn.c3", "POC corr. filter 1 + 2, TFF blanks", label),
         label = gsub("s.cn", "POC uncorr.", label),
         time = ifelse(key %in% c("i.cn", "i.cn.c1", "i.cn.c3"), "Initial", "Stationary"),
         value = ifelse(time == "Initial" & Bottle %in% c("B", "D", "F"), NA, value)) %>% 
  ggplot(aes(x = Season, y = value)) + 
  geom_boxplot(width = 0.3) +
  geom_jitter(aes(color = time, shape = gf75.flag), width = 0.1,  size = 4, alpha = 0.6) + 
  ...
  #scale_shape_manual(values = c("OK" = 16, "Initial Questionable" = 2, "Stationary Questionable" = 3)) +
  custom_theme() +
  facet_grid(facet_depth~type+factor(label, levels = c("POC uncorr.", "POC corr. filt 2, all or only TFF blanks", "POC corr. filter 1 + 2, TFF blanks" )), scales = "free") + 
  ggtitle("C:N ratios")

```

C:N ratios of POC-corrected samples appear to be more realistic than uncorrected values. However, even exluding questionable measurements, N3 still has an outlier for the initial condition. The points on the line intersecting the max of the y-axis represent "Inf" values, where PON values are 0. 

Summary Statistics including N3:
```{r echo = FALSE, results = 'asis'}
fg_cell.qc %>% 
...
fg_cell.qc %>% 
  filter(Treatment == "Control") %>% 
  select(i.ccf.c1:i.ccf.c3) %>%
  summary()
```

Summary Statistics excluding N3:
```{r echo = FALSE, results = 'asis'}
fg_cell.qc %>% 
  filter(Treatment == "Control", Cruise != "AT38") %>% 
  select(i.cn:i.cn.c3) %>% 
  summary()
```
...
While the medians are within the range, the mean aveage contents of carbon are  higher than those reported by Fukuda et al. (1998): $$12.4 \pm 6.3$$ fg C cell^-1^
...
fg_cell.qc %>%
  select(Season:Bottle, i.ccf.c1, i.ccf.c3, s.ccf.c1, s.ccf.c3 ) %>% 
  gather(key, ccf, i.ccf.c1:s.ccf.c3) %>% 
  mutate(time = ifelse(str_detect(key, "i"), "Initial", "Stationary" ),
         corr = ifelse(str_detect(key, "c1"), "POC corr. filt 2, all or only TFF blanks", "POC corr. filter 1 + 2, TFF blanks"),
  ...
  facet_grid(facetlabel+type~time+corr, scales = "free")
...
fg_cell.qc %>%
  ...
  left_join(., fg_cell.qc %>% 
              ...
              mutate(time = ifelse(str_detect(key, "i"), "Initial", "Stationary" ),
                     corr = ifelse(str_detect(key, "c1"), "POC corr. filt 2, all or only TFF blanks", "POC corr. filter 1 + 2, TFF blanks"),
                     corr = ifelse(key %in% c("i.poc.um", "s.poc.um"), "Not Corrected", corr)) %>% 
              drop_na(poc) %>% 
              select(-key)) %>% 
  mutate(facetlabel = paste(Depth, "m")) %>% 
  #####PLOT####
...
fg_cell.qc %>%
  ...
  left_join(., fg_cell.qc %>% 
              ...
              mutate(time = ifelse(str_detect(key, "i"), "Initial", "Stationary" ),
                     corr = ifelse(str_detect(key, "c1"), "POC corr. filt 2, all or only TFF blanks", "POC corr. filter 1 + 2, TFF blanks"),
                     corr = ifelse(key %in% c("i.cn", "s.cn"), "Not Corrected", corr)) %>% 
              drop_na(cn) %>% 
              select(-key)) %>% 
  mutate(facetlabel = paste(Depth, "m")) %>% 
  #####PLOT####
...
bge_summary.plot.data %>%    
  ...
  # geom_label(aes(label = n, fill = factor(appr, levels = bge_lvls)), color = "white", size = 3, alpha = 0.7) + 
  labs(x = "", y = expression(italic("BGE")), colour = expression(italic(""))) + 
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
  facet_grid(facet_depth + type~factor(Season, levels = levels) , scales = "free") +
...
Excluding non-control and 200 m experiments.
