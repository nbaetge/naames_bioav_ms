...
bge_summary %>% 
  ...
  add_tally() %>% 
  ungroup() %>% 
  select(Season, type, Bottle,  appr, ave_bge:med_bge, n) %>% 
  distinct() %>% 
  arrange(type, Season) %>% 
  mutate_at(vars(ave_bge:med_bge), round, 2) %>% 
  filter(!appr %in% c("bge.p.bc.dyn.ccf.c1", "bge.p.bc.dyn.ccf.c2", "bge.p.bc.dyn.ccf.c3", "bge.ph.bc.dyn.ccf.c1", "bge.ph.bc.dyn.ccf.c2", "bge.ph.bc.dyn.ccf
```
...
Excluding non-control and 200 m experiments....these are the experiments that have ∆DOC values >= 1.5 µmol C L^-1^ and acceptable BGEs. 
...
```{r}
bge_summary.plot.data %>% 
  filter(type == "Control", Depth == 10) %>% 
  select(Cruise, Station,  Subregion, Bottle, gf75.flag) %>% 
  distinct() %>% 
  arrange(Cruise, Station, Bottle)
```
...
bge_summary.plot.data %>% 
  ...
  scale_y_continuous(breaks = pretty_breaks()) +
  custom_theme() +
  # facet_grid(factor(Subregion, levels = levels) ~ factor(Season, levels = levels) , scales = "free") +
  facet_grid(gf75.flag + Subregion ~ factor(Season, levels = levels) , scales = "free") +
  ggtitle("Bacterial Growth Efficiences") +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
