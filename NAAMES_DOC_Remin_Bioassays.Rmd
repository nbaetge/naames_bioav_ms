...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 12, fig.align = "center", warning = FALSE}
bcd %>% 
  select(Cruise:Station, degree_bin, ave_NPP, sd_NPP, int.bcd.cr_bge.ez, int.bcd.ez) %>% 
  pivot_longer(cols = c(ave_NPP, int.bcd.cr_bge.ez, int.bcd.ez), names_to = "rate", values_to = "value") %>% 
  mutate(rate = ifelse(rate %in% c("int.bcd.cr_bge.ez", "int.bcd.ez"), "BCD", "NPP"),
         degree_bin = as.character(degree_bin)) %>% 
  group_by(Cruise, Station, rate) %>% 
  mutate(ave = mean(value)/10^3,
         sd = sd(value)/10^3,
         sd = ifelse(rate == "NPP", sd_NPP/10^3, sd )) %>% 
  ungroup() %>% 
  ##PLOT
